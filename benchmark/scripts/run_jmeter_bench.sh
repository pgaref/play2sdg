...
do
	...
	echo "Starting Play"
	parallel-ssh -H "$_WORKERS" "cd $SCRIPTS_HOME/ && screen -dm -S 'play_run' ./run_play.sh"
	...
	if [ "$STATS" == "1" ]; then
		echo "Starting $STATS_SCRIPT on workers: "$_WORKERS""
		parallel-ssh -H "$_WORKERS" "cd $SCRIPTS_HOME/ && screen -dm -S 'stats' python $STATS_SCRIPT -i ${IFACE} -f ${STATS_FILENAME}"
	fi
	...
	stop_play_app	$_WORKERS
	echo "Stopping $STATS_SCRIPT on workers"
	parallel-ssh -H "$_WORKERS" "killall -u $USER -SIGINT python"
