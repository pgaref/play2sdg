...
############################################################
RUN='w16_seep_multithread_kv_no-cc-v4'
#Stats collection variable
STATS='1'
#Variable to clear caches
CACHE_CLEAR='0'
SEEP_MASTER_CMD="tmux new -d -s master '/home/pg1712/virtual_share/seep-install/seep-master/bin/seep-master -properties.file=/home/pg1712/virtual_share/seep-install/pgaref-config/master.properties'"
...
do
	...
	for worker in ${PLAY_WORKERS}; do
		echo "Starting SEEP"
		ssh $worker "$SEEP_MASTER_CMD"
		printf "Wait SEEP to settle..."
		sleep 5
		echo $worker `ssh $worker "tmux ls"`
		sleep 5
		ssh $worker "tmux send-keys -t master '1' Enter"
		ssh $worker "tmux send-keys -t master '1' Enter"
		sleep 5
		ssh $worker "tmux send-keys -t master '2' Enter"
		printf "done\n"
	done
...
#		ssh $worker 'rm /home/pg1712/play2sdg-1.0-SNAPSHOT/RUNNING_PID'
		ssh $worker "tmux kill-session -t master"
        printf "done\n"
	done
